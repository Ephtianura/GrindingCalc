<div align="center">

<img alt="grid-calc-banner" src="./docs/banner.png"/>

# ⚙️ Grinding Calculator – Інженерна програма автоматизації розрахунку режимів шліфування

[![License](https://img.shields.io/badge/.exe-Завантажити-35439F?style=for-the-badge)](https://github.com/Ephtianura/GrindingCalc/releases/tag/v1.1)
[![License](https://img.shields.io/badge/📜_License-MIT-35439F?style=for-the-badge)](LICENSE)

</div>

> _«Достатньо просто ввести дані – програма порахує сама.»_

Прикладне інженерне ПЗ для автоматизації високоточних розрахунків режимів шліфування матеріалів. Застосунок переводить складний масив нормативно-довідкових таблиць та понад 30 наукових формул теорії різання у швидкий покроковий алгоритм. Автоматично валідує вхідні параметри, перевіряє можливість різання та генерує підсумкові карти з можливістю експорту в MS Excel.

<details open>
<summary><b>Навіщо це?</b></summary>

Додаток створювався на запит викладачів та інженерів-технологів навчального закладу.

Традиційний розрахунок режимів шліфування вручну – це надто тривалий та рутинний процес:

1. **Втрата часу:** Розрахунок одного тех. процесу за допомогою формул, таблиць та паспорта верстата займав у студентів та інженерів від **1.5 до 2 годин**.
2. **Людський фактор та ризики:** Найменша помилка під час вибору коефіцієнта з довідника або обчислення степеня зводила нанівець усі попередні розрахунки, змушуючи проходити весь шлях заново. А якщо помилку не помічали на папері – це створювало реальні ризики зіпсувати деталь або пошкодити обладнання при роботі за верстатом.
3. **Складність перевірки:** Викладачу доводилося вручну перераховувати кожну формулу, щоб знайти, де саме студент припустився помилки.

Вся концепція покрокового алгоритму, логіка та дизайн інтерфейсу розроблялися у тісній співпраці із замовником та були максимально узгоджені під їхні вимоги. Також було вирішено прохання легкого розповсюдження: додаток упаковано в простий інсталятор, що дозволило розгортати його на будь-якому комп'ютері.

> **Цікавий факт:** Під час тестування додаток видавав результат, який розходився прикладами. Як виявилося пізніше, програма була абсолютно права – у самій методичці була закрадена математична помилка. Програма не помиляється.

#### 📊 Результат впровадження

- ⏳ Час отримання готової тех. карти скоротився з **120 хвилин до ~5 хвилин**.
- 🚫 Повністю виключено помилки у виразах та людський фактор.
- ✅ Продукт був успішно переданий та впроваджений у навчальний процес профільних дисциплін для студентів та викладачів.

</details>

<details open>

<summary><h3>Зміст</h3></summary>

- [🛠️ Стек](#tech-stack)
- [📸 Галерея](#gallery)
- [🔬Задіяні формули](#formulas)

</details>

<details open>
<summary><h2 id="tech-stack">🛠️ Технологічний Стек</h2></summary>

---

![](https://img.shields.io/badge/C%23-5C2D91?style=flat-square&logo=csharp&logoColor=white) <!-- C# -->
![](https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=.net&logoColor=white) <!-- .NET -->
![](https://img.shields.io/badge/Windows_Forms-0078D4?style=flat-square&logo=windows&logoColor=white) <!-- WinForms -->
![](https://img.shields.io/badge/Visual_Studio_2022-6F50A5?style=flat-square&logo=visualstudio&logoColor=white) <!-- Visual Studio -->
![](https://img.shields.io/badge/OOP-6D6D6D?style=flat-square) <!-- OOP -->

</details>

<details open>
<summary><h2 id="gallery">📸 Галерея</h2></summary>

---

<div align="center">

<img height="400" alt="Вибір виду шліфування" src="./docs/demo.webp"/>
<p align="center"><small><i>Коротка демонстрація можливостей застосунку</i></small></p>

<img height="400" alt="Вибір виду шліфування" src="./docs/grinding-type.png"/>
<p align="center"><small><i>Крок 1: Вибір базового виду обробки</i></small></p>

<img height="400" alt="Вибір способу шліфування" src="./docs/grinding-method.png"/>
<p align="center"><small><i>Крок 2: Деталізація способу шліфування (Для кожного виду різні варіанти)</i></small></p>

<img height="400" alt="Введення вихідних даних" src="./docs/inpit-data-1.png"/>
<p align="center"><small><i>Крок 3: Введення геометричних та ін. параметрів за нормативними таблицями</i></small></p>

<img height="400" alt="Розрахунок режимів та коефіцієнтів" src="./docs/inpit-data-2.png"/>
<p align="center"><small><i>Крок 4: Задання останніх параметрів та автоматична перевірка можливості різання</i></small></p>

<img height="400" alt="Валідація та помилка некоректного введення" src="./docs/error-data.png"/>
<p align="center"><small><i>Попередження при виході параметрів за допустимі межі верстата</i></small></p>

<img height="400" alt="Підсумкові результати розрахунку" src="./docs/results.png"/>
<p align="center"><small><i>Крок 5: Сформований підсумковий екран із усіма необхідними параметрами для різання</i></small></p>

<img height="400" alt="Експорт результатів у MS Excel" src="./docs/excel.png"/>
<p align="center"><small><i>Згенерований підсумковий звіт у форматі Microsoft Excel (.xlsx)</i></small></p>

<img height="400" alt="Вікно Про програму та інструкція" src="./docs/about.png"/>
<p align="center"><small><i>Вікно «Про додаток» з інформацією про версію, автора та вбудованою покроковою інструкцією користувача</i></small></p>

</div>

</details>

<!-- 🔬Задіяні формули -->
<details open>

<summary><h2 id="formulas">🔬Задіяні формули</h2></summary>

---

### Вхідні дані

| Ідентифікатор | Діапазон значень    | Пояснення                                  |
| ------------- | ------------------- | ------------------------------------------ |
| -             | Типи шліфування     | Тип шліфування                             |
| -             | Способи обробки     | Спосіб шліфування                          |
| l             | 0.0 - 1000.0        | Довжина шліфування                         |
| d             | 0.0 - 1000.0        | Ширина шліфування                          |
| Q             | 0.0 - 100.0         | Кількість одночасно оброблюваних заготовок |
| h             | 0.0 - 10.0          | Припуск обробки на сторону                 |
| nкр           | 0 - 100000          | Частота обертання шліфувального круга      |
| -             | ПП, ПВ і т.д.       | Тип шліфувального круга                    |
| D             | 0.0 - 1000.0        | Діаметр шліфувального круга                |
| H             | 0.0 - 100.0         | Ширина шліфувального круга                 |
| kW            | 0.0 - 100.0         | Потужність приводу                         |
| St            | 0.0 - 1000.0        | Ширина на глибину шліфування               |
| Sp            | 0.0 - 100.0         | Радіальна подача                           |
| So            | 0.0 - 100.0         | Поздовжня подача                           |
| Sx            | 0.0 - 100.0         | Поперечна подача                           |
| -             | Ручний/Автоматичний | Тип стола верстата                         |
| -             | Попередня/Остаточна | Попередня обробка                          |
| V3            | 0 - 10000           | Швидкість обертання заготовки              |
| nd            | 0.0 - 10000.0       | Частота обертання деталі                   |
| ndd           | 0.0 - 10000.0       | Уточнена частота обертання деталі          |
| CN            | 0.0 - 100.0         | Розрахунковий параметр                     |
| r             | 0.0 - 100.0         | Розрахунковий параметр                     |
| x             | -1000.0 - 1000.0    | Розрахунковий параметр                     |
| y             | -1000.0 - 1000.0    | Розрахунковий параметр                     |
| q             | 0.0 - 1000.0        | Розрахунковий параметр                     |
| z             | 0.0 - 1000.0        | Розрахунковий параметр                     |


### Перелік формул

---

**Швидкість головного руху різання (обертання шліфувального круга)**

$$
V_{\text{кр}} = \frac{\pi D_{\text{кр}} n_{\text{кр}}}{1000 \cdot 60} \quad \text{(2.1)}
$$

**Частота обертання деталі**

$$
n_{\text{д}} = \frac{1000 V_{\text{д.окр}}}{\pi d} \quad \text{(2.2)}
$$

**Швидкість поздовжнього ходу стола**

$$
V_{S_{\text{позд}}} = \frac{n_{\text{д.д}} S_o}{1000} \quad \text{(2.3)}
$$

**Потужність різання при шліфуванні периферією круга з поздовжньою подачею**

$$
N_{\text{різ}} =
C_N \cdot V_{\text{з.окр}}^{r}
\cdot S_t^{x}
\cdot S_o^{y}
\cdot d^{q} \quad \text{(2.4)}
$$

**Потужність різання при шліфуванні периферією круга з радіальною подачею (врізанням)**

$$
N_{\text{різ}} =
C_N \cdot V_{\text{з.окр}}^{r}
\cdot S_p^{y}
\cdot d^{q}
\cdot b^{z} \quad \text{(2.5)}
$$

**Потужність різання при шліфуванні торцем круга**

$$
N_{\text{різ}} =
C_N \cdot V_{\text{з.окр}}^{r}
\cdot S_t^{x}
\cdot b^{z} \quad \text{(2.6)}
$$

**Умова допустимої потужності різання**

$$
N_{\text{різ}} \leq N_{\text{шп}} \quad \text{(2.7)}
$$

**Потужність шпинделя**

$$
N_{\text{шп}} = N_{\text{дв}} \cdot \eta \quad \text{(2.8)}
$$

**Основний технологічний час при зовнішньому круглому шліфуванні з поздовжньою подачею**

$$
T_o =
\frac{L_{\text{р.х}} \cdot h}
{S_o \cdot S_t \cdot n_{\text{зд}}}
\cdot K \quad \text{(2.9)}
$$

**Довжина робочого ходу інструменту**

$$
L_{\text{р.х}} = l + 0{,}5B_{\text{кр}} \quad \text{(2.10)}
$$

**Основний технологічний час при зовнішньому круглому та безцентровому шліфуванні з радіальною подачею (врізанням)**

$$
T_o = \frac{h}{n_{\text{д.д}} \cdot S_p} \cdot K \quad \text{(2.11)}
$$

**Основний технологічний час при безцентровому круглому шліфуванні з поздовжньою подачею**

$$
T_o = \frac{h}{S_o \cdot S_p} \cdot K \quad \text{(2.12)}
$$

### Розрахунок режимів різання при шліфуванні отвору і торця

**Швидкість головного руху різання (обертання шліфувального круга)**

$$
V_{\text{кр}} = \frac{\pi D_{\text{кр}} n_{\text{кр}}}{1000 \cdot 60} \quad \text{(2.13)}
$$

**Частота обертання деталі**

$$
n_{\text{д}} = \frac{1000 \cdot V_{\text{д.окр}}}{\pi \cdot d} \quad \text{(2.14)}
$$

**Швидкість руху колової подачі**

$$
V_{S_{\text{кол}}} = \frac{\pi d n_{\text{д.д}}}{1000} \quad \text{(2.1)}
$$

**Швидкість руху поздовжньої подачі**

$$
V_{S_{\text{позд}}} = \frac{S_o n_{\text{д.д}}}{1000} \quad \text{(2.16)}
$$

**Потужність різання при шліфуванні периферією круга з поздовжньою подачею**

$$
N_{\text{різ}} =
C_N \cdot V_{S_{\text{кол}}}^{r}
\cdot S_t^{x}
\cdot S_o^{y}
\cdot d^{q} \quad \text{(2.17)}
$$

**Потужність різання при шліфуванні периферією круга з радіальною подачею**

$$
N_{\text{різ}} =
C_N \cdot V_{S_{\text{кол}}}^{r}
\cdot S^{y}
\cdot d^{q}
\cdot b^{z} \quad \text{(2.18)}
$$

**Потужність різання при шліфуванні торцем круга**

$$
N_{\text{різ}} =
C_N \cdot V_{S_{\text{кол}}}^{r}
\cdot S_t^{x}
\cdot b^{z} \quad \text{(2.19)}
$$

**Умова допустимої потужності різання**

$$
N_{\text{різ}} \leq N_{\text{шп}} \quad \text{(2.20)}
$$

**Потужність шпинделя**

$$
N_{\text{шп}} = N_{\text{дв}} \cdot \eta \quad \text{(2.21)}
$$

**Основний технологічний час при шліфуванні з поздовжньою подачею**

$$
T_o = \frac{2L_{\text{р.х}}h}{V_{S_{\text{позд}}}S_t} \quad \text{(2.22)}
$$

**Основний технологічний час при шліфуванні з радіальною подачею (врізанням)**

$$
T_o = \frac{h}{n_{\text{д}}S_p} \cdot K \quad \text{(2.23)}
$$

**Довжина робочого ходу інструменту**

$$
L_{\text{р.х}} = l + 0{,}5B_{\text{кр}} \quad \text{(2.24)}
$$

### Розрахунок режимів різання при шліфуванні площини

**Швидкість шліфувального круга**

$$
V_{\text{кр}} = \frac{\pi D_{\text{кр}} n_{\text{кр}}}{1000 \cdot 60} \quad \text{(2.25)}
$$

**Потужність різання при шліфуванні периферією круга з поздовжньою подачею**

$$
N_{\text{різ}} =
C_N \cdot V_{S_{\text{позд}}}^{r}
\cdot S_t^{x}
\cdot S_o^{y}
\cdot B_{\text{кр}}^{0{,}25} \quad \text{(2.26)}
$$

**Потужність різання при шліфуванні торцем круга**

$$
N_{\text{різ}} =
C_N \cdot V_{S_{\text{позд}}}^{r}
\cdot S_t^{x}
\cdot B^{z} \quad \text{(2.27)}
$$

**Умова допустимої потужності різання**

$$
N_{\text{різ}} \leq N_{\text{шп}} \quad \text{(2.28)}
$$

**Потужність шпинделя**

$$
N_{\text{шп}} = N_{\text{дв}} \cdot \eta \quad \text{(2.29)}
$$

**Основний технологічний час при шліфуванні з поздовжньою подачею**

$$
T_o = \frac{2L_{\text{р.х}}h}{V_{S_{\text{позд}}}S_t} \quad \text{(2.30)}
$$

**Основний технологічний час при шліфуванні з радіальною подачею (врізанням)**

$$
T_o = \frac{HLh}{1000V_{S_{\text{позд}}}S_xS_tq} \cdot K \quad \text{(2.31)}
$$

**Переміщення шліфувального круга в напрямку поперечної подачі**

$$
H = B_{\text{з}} + B_{\text{кр}} + 5 \quad \text{(2.32)}
$$

**Довжина поздовжнього ходу стола**

$$
L = L_{\text{з}} + 10 \quad \text{(2.33)}
$$

**Основний технологічний час при шліфуванні торцем круга**

$$
T_o = \frac{HLh}{1000V_{S_{\text{позд}}}S_xS_tq} \cdot K \quad \text{(2.34)}
$$

</details>

